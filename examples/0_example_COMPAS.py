...
# hyb_model.fit(X_train, y_train, 100, random_state=random_state_param)
...
# Train the hybrid model
hyb_model.fit(X_train, y_train, 500, random_state=random_state_param, print_progress=True)
overall_accuracy, output_rules, _, _, cover_rate = hyb_model.test(X_test, y_test)

# Print the RuleList
print(" ACC ,  C  ,  Rule")
for (i,j,k) in zip(overall_accuracy, output_rules, cover_rate):
